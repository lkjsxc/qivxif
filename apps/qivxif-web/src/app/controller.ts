...
export async function createController(root) {
  const state = initialWorkspaceState();
  const store = await openLocalStore();
  const listeners = new Set();

  const notify = () => {
    const actions = actionsFor(root, store, state, notify);
  ...
  return {
    async start() {
      ...
      installKeyboardShortcuts(() => actionsFor(root, store, state, notify), state);
    ...
    subscribe(listener) {
      ...
      listener(state, actionsFor(root, store, state, notify));
