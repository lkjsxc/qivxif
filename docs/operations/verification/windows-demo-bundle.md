...
dist/windows/demo/qivxif-client-desktop.exe
...
dist/windows/demo/run-desktop-client.cmd
...
- `run-desktop-client.cmd` opens the native desktop client against
  `127.0.0.1:4443` with `--tls local-compose`.
- `run-desktop-client.cmd` pauses after the client exits.
- `config/server.toml` binds to `127.0.0.1:4443` and uses `data` as `data_dir`.

## Verification Requirements

- All three `.exe` files exist and are non-empty.
- Config, launchers, README, manifest, and checksums exist.
- `checksums.txt` includes executables, launchers, and config.
